...
func main() {
        ...
        for _, channel := range channels {
                ...
                for i, msg := range messages {
                        if config.Debug {
                                // Output the message being reviewed
                                fmt.Printf("Reviewing message: Author=%s, Timestamp=%s, Content=%s\n", msg.Author.Username, msg.Timestamp, msg.Content)
                        }
                        ...
                        if totalMessages > 0 && (i+1)%10 == 0 {
                                percentComplete := float64(i+1) / float64(totalMessages) * 100
                                fmt.Printf("Processed %.2f%% of messages in channel %s\n", percentComplete, channel.Name)
                        }
                }
        }
